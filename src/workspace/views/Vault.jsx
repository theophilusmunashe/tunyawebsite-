...
import { Button, Empty, Kicker, Modal } from "../ui.jsx";
...
export default function Vault() {
  ...
  const ingest = async (list) => {
    const chosen = folder === "All" ? "House" : folder;
    for (const file of list) {
      if (file.size > 20 * 1024 * 1024) {
        toast(`${file.name} is over 20 MB — ask the house to split it.`);
        continue;
      }
      await addFile(file, chosen, you?.name);
    }
    toast(list.length > 1 ? `${list.length} files into the Vault` : "Filed in the Vault");
  };
  ...
  return (
    <div>
      <div className="ws-page-head">
        <div>
          <Kicker>The Vault</Kicker>
          <h1>The house cupboard.</h1>
          <p className="ws-lede">Contracts, itineraries, permits, guest packs and supplier rates — uploaded once, open to everyone who holds the house key. This cupboard lives in this browser until you export a workspace pack from Settings and restore it on another desk.</p>
        </div>
      ...
      >
        Drop files here. They go into {folder === "All" ? "House" : folder}.
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 8 }}>
        ...
        <input className="ws-search" placeholder="Search the cupboard" value={q} onChange={(e) => setQ(e.target.value)} />
      </div>

      {visible.length === 0 && <Empty>Nothing in this drawer yet. The first contract you drop becomes the start of the house memory.</Empty>}

      <div className="ws-files">
        {visible.map((f) => (
          <div className="ws-file" key={f.id}>
            <Kicker>{f.folder}</Kicker>
            ...
            <div className="ws-actions" style={{ marginTop: "auto" }}>
              ...
              <Button kind="ghost" className="slim" onClick={() => downloadFile(f.id)}>Keep</Button>
      ...
      {preview && (
        <Modal title={preview.name} kicker={preview.folder} onClose={() => { URL.revokeObjectURL(preview.url); setPreview(null); }}>
